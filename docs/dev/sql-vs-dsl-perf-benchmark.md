...
| P0  | 统计信息注入                       | 独立工作流 | 20-40 | 最高 ROI，让 Calcite CBO 生效 |
| P1  | COALESCE                     | 函数注册  | 1-2   | Calcite 原生支持            |
| P1  | DATE_HISTOGRAM               | 聚合修复  | 3-5   | 时序分析基础                  |
| P2  | 3表+ JOIN / EXISTS            | 计划级路由 | 8-13  | 复用 UNION 三步模式           |
| P3  | JOIN+GROUP BY / 子查询+GROUP BY | 依赖 P2 | 10-16 | schema 验证               |
| P4  | 标量子查询                        | 表达式级  | 8-12  | 复杂度最高                   |
| P5  | CTE                          | 语句级   | 15-25 | 文法+AST+作用域              |

- UNION 扩展的"三步模式"（AstBuilder 不抛异常 → shouldUseCalcite 路由 → CalciteRelNodeVisitor 实现）仅适用于**计划级路由**扩展；函数注册、聚合修复等需不同模式
- 快速收益（P0+P1）：24-47 人天即可补齐统计信息注入、COALESCE、DATE_HISTOGRAM
- 全部覆盖（P0-P5）：65-113 人天
...
**能力层面**：建议优先推进 P0（统计信息注入）和 P1（COALESCE、DATE_HISTOGRAM），以 24-47 人天的投入补齐最高 ROI 的能力缺口。CTE 和标量子查询可后续迭代。

**选型决策**：本文档为性能维度的技术分析报告，不构成选型决策。选型需叠加以下独立维度后综合判断：① 安全性（SQL 注入、权限控制、审计日志）；② 运维（升级兼容性、监控完备性）；③ 生态兼容（JDBC/ODBC 驱动、BI 工具对接）；④ 并发负载与故障恢复（4.8 节 I 组已设计但未执行）。
