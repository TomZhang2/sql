...
1. **SQL 最终都翻译成 DSL**——V2 和 Legacy V1 路径确实如此；但 **Calcite 路径不完全如此**：可下推部分（filter/agg/sort/limit）翻译为 DSL，不可下推部分通过 Enumerable 算子在协调节点内存计算（Janino codegen），不翻译为 DSL。差异在上层翻译开销 + 内存计算开销
...
> 本章记录在 3 节点 OpenSearch 集群上执行的 SQL vs DSL 性能对比实测数据。1M 轻查询/重查询（§5.2/§5.3）、10M 控制变量场景（§5.2.1/§5.3.1a，相同查询仅变数据量）、10M 独立场景（§5.4，4.4 节方案）、G0 纯翻译开销基线（§5.5）。不包含 4.8 节生产关键场景。
...
| 测试索引（10M）           | `perf_test_10m`：6 primary shard + 1 replica = 12 shard，10,000,000 文档，5 GB（5.2.1/5.3.1a/5.4 节使用）                                                                              |
...
#### 5.3.1a 重查询实测数据（10M 数据，相同查询控制变量）
...
3. **中基数聚合（~10K桶）性能边界化**——C1-H 劣化 83%，绝对开销 65ms，接近瓶颈拐点。⚠️ SQL composite 返回全量桶、DSL terms size=1000 返回 Top 1000，结果可能因 `doc_count_error` 有底部排名差异（见 §4.9）
...
6. **技术结论**：在单连接串行、4GB heap、回环网络条件下，SQL 插件在点查/全文/分页/低基数聚合场景性能可接受；高基数聚合（GROUP BY 高基数字段 + LIMIT）应使用 DSL 或 PPL `stats`（走 Calcite pushdown）。**注：此结论基于单连接串行测试，并发负载与故障恢复未验证（见 4.8 节 I 组）；4GB heap 非生产 8GB，H1 的 140× 劣化在生产环境下可能不同（见 §5.8 局限性 2）；PPL `stats` 缓解方案未实测**
...
|:------:| ----------------- | ----- |:-----:|:----------:| --------------------------------------------------------------------- |
| **P1** | COALESCE          | 函数注册  | 1-2   | ~5         | Calcite 原生支持，仅需声明 operator + enum + grammar                        |
| **P1** | DATE_HISTOGRAM    | 聚合修复  | 3-5   | ~65-130    | UDF 实现（~45-110 LOC）+ resolver 注册（~20 LOC）                          |
| **P2** | 3表+ JOIN          | 计划级路由 | 5-8   | ~140-170   | 复用 UNION 三步模式（UNION 参考：~171 LOC）                                  |
| **P2** | EXISTS 子查询        | 计划级路由 | 3-5   | ~140-170   | 同三步模式，SqlV2QueryParser 有参考                                         |
| **P3** | JOIN + GROUP BY   | 依赖 P2 | 5-8   | ~50-80     | 在 JOIN 基础上增加 schema 解析+字段名冲突处理                                    |
| **P3** | 子查询 + 外层 GROUP BY | 依赖 P2 | 5-8   | ~50-80     | 在 JOIN 基础上增加派生表 schema 传播                                         |
| **P3** | 统计信息注入            | 独立工作流 | 20-40 | —          | 让 Calcite CBO 生效，但当前 SQL 默认走 V2 不经 Calcite，需 Calcite 成为 SQL 默认引擎后才有价值 |
| **P4** | 标量子查询             | 表达式级  | 8-12  | ~60-100    | AstExpressionBuilder + RexSubquery，比三步模式复杂                         |
| **P5** | CTE               | 语句级   | 15-25 | ~150-250   | ANTLR 文法规则 + AST 节点 + 作用域管理                                      |
...
| 优先级 | 特性                           | 模式    | 人天    | 代码量 (LOC) | 理由                                                                    |
|:---:| ---------------------------- | ----- |:-----:|:----------:| --------------------------------------------------------------------- |
| P1  | COALESCE                     | 函数注册  | 1-2   | ~5         | Calcite 原生支持，仅需声明 operator                                                     |
| P1  | DATE_HISTOGRAM               | 聚合修复  | 3-5   | ~65-130    | UDF + resolver 注册                                                       |
| P2  | 3表+ JOIN / EXISTS            | 计划级路由 | 8-13  | ~140-170×2 | 复用 UNION 三步模式                                                           |
| P3  | JOIN+GROUP BY / 子查询+GROUP BY | 依赖 P2 | 10-16 | ~50-80×2   | schema 验证                                                             |
| P3  | 统计信息注入                       | 独立工作流 | 20-40 | —          | 让 Calcite CBO 生效，但当前 SQL 默认走 V2 不经 Calcite，需 Calcite 成为 SQL 默认引擎后才有价值 |
| P4  | 标量子查询                        | 表达式级  | 8-12  | ~60-100    | 复杂度最高                                                                 |
| P5  | CTE                          | 语句级   | 15-25 | ~150-250   | 文法+AST+作用域                                                            |
