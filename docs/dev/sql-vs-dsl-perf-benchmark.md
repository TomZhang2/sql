...
> 本附录记录 2026-07-20 在 3 节点 OpenSearch 集群上执行的 SQL vs DSL 性能对比实测数据。测试方案见第四章 4.2 节（轻查询场景）+ G0 组（纯翻译开销基线），不包含 4.8 节生产关键场景。
...
| 测试索引 | `perf_test`：6 primary shard + 1 replica = 12 shard，1,000,000 文档，390 MB |
| 分片分布 | 每节点 4 shard（2 primary + 2 replica），约 33 MB/shard |
| 段（segment）数 | 39 个（未执行 forcemerge，保持导入后自然状态） |
...
| 测试时间 | 2026-07-20 20:12-20:15 UTC |
...
> ⚠️ **实测值低于预期区间**：G0-1 实测 0.76ms（预期 2-7ms），G0-2 实测 0.84ms（预期 3-10ms）。原因分析见 B.4 节第 1 点。
...
**系统性偏差总结**：
- **DSL 延迟普遍低于预期 25-84%**：3 节点集群 + M4 Pro + SSD + JDK 25 的组合性能远超预期基线（预期基于单节点/低配硬件）
- **SQL 绝对开销低于预期**：8 个场景中 6 个低于预期下界，但 2 个（A1、D3）在预期区间内
- **开销占比基本在预期内**：8 个场景中 7 个在预期区间，仅 A1 因 DSL 基线过低导致占比超上界
...
9. **数据量局限**：1M 文档 390MB，单分片 ~33MB。生产 10M-100M 文档时分片数据量 330MB-3.3GB，DSL 基线显著升高，SQL 劣化比例会降低。本测试无法直接外推至 10M+ 场景（需 T 组趋势验证）
10. **无 p999 和长尾分析**：200 轮样本无法可靠测量 p999。生产 SLO 通常关注 p99.9，需 ≥10000 轮样本

---

> **报告生成时间**：2026-07-20 20:15 UTC
> **测试脚本**：`/tmp/os-bench/benchmark.py`（含 `bench()` / `bench_explain()` / `bench_ppl_profile()` 三个测量函数）
> **原始数据**：`/tmp/os-bench/results.json`（JSON 格式，含全部 p50/p95/p99/max/mean 统计）
> **集群状态**：测试后保留运行（node-1 @ 9201, node-2 @ 9202, node-3 @ 9203），可供复查
