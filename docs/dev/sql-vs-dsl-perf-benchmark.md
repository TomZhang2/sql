...
| D3 大结果集 | ~0.82 (估)           | 9.31       | ~8.49   | **序列化(1000行) 占主导**，约 8ms |

- 轻查询差异 1.17-1.65ms：`JdbcResponseFormatter` 序列化 + sql-worker 线程调度 + HTTP 往返
- D3 差异 ~8.49ms：序列化 1000 行 JSON 占 ~90%，与 4.7 节"序列化开销可达 10-50ms"预期一致
- 网络：3 节点同机回环 <0.1ms，生产环境跨节点 0.5-2ms 差异会更大
- **交叉验证**：G0-1 `_explain` 1.05ms + 序列化/调度 ~1.17ms ≈ A1 端到端开销 2.22ms，数学闭合（注：此为间接推断——序列化/调度 1.17ms 由端到端减 `_explain` 反推，非独立测量，见 §5.8 局限性 4）
...
**G3 高基数聚合分析**：SQL 74ms / DSL 9.2ms（8 倍），与 1M C1-H（78ms / 13ms，6 倍）相比 SQL 时间相近但 DSL 更快（因 G3 有时间过滤扫描文档更少）。控制变量对比中 C1-H 10M（116ms / 17ms，6.6 倍）因查询和过滤条件不同不可直接与 G3 比较，但 G3 的 87.6% 占比与 C1-H 10M 的 85.2% 一致，印证高基数聚合场景劣化不随数据量改善。
...
| 维度         | 4.2 轻查询（1M，≤10行）         | 4.2 轻查询（10M，相同查询） | 4.3 重查询（1M，5K-10K行） | 4.3 重查询（10M，相同查询） | 4.4 10M 独立场景         | 趋势                         |
| ---------- | ------------------------ | ----------------- | ------------------- | ----------------- | -------------------- | -------------------------- |
...
5. **"大数据量劣化比例小"假设在序列化主导和低基数聚合场景成立**（控制变量验证：大结果集 ↓8-13pp、低基数聚合 ↓2-9pp）；在高基数聚合场景不成立（C1-H ↑2pp，composite 分页成本随数据量上升）
