...
**翻译开销**：G0 实测 ANALYZE 阶段 0.76-0.84ms（G0-1 点查 / G0-2 聚合），显著低于 4.7 节预期 2-10ms（基于保守硬件假设，见 §4.7 预期区间说明）。`_explain` 端点响应时间 0.65-0.74ms，与方法 B 一致量级。

> ⚠️ **范围说明**：G0 测量的是 ANALYZE 阶段（解析 + 构建 RelNode），**不含 Calcite Volcano 优化器**（OPTIMIZE 是独立阶段，见 5.5 节）。V2 路径无 Calcite 优化器，ANALYZE 即为完整翻译开销。但 G0 方法 B 测的是 PPL/Calcite 路径的 ANALYZE，V2 路径的翻译开销**未直接测量**——下文"端到端分解"是间接推断，非直接验证。Calcite 路径（E 组 UNION）完整翻译开销 = ANALYZE + OPTIMIZE，G0 仅测了 ANALYZE 部分，但轻查询 OPTIMIZE <0.1ms 影响可忽略。复杂 UNION 查询的 OPTIMIZE 可能更高（2.3 节预期 2-8ms），G0-3 未实测。

低于预期的主要原因：① Apple M4 Pro + JDK 25 硬件性能强劲，ANTLR 解析全链路 <1ms；② 查询简单（A1 仅 1 个 WHERE + LIMIT，G0-2 仅 4 桶），ANTLR 解析树节点数少。

**端到端开销分解**：G0 纯翻译开销与端到端开销的差异随结果集大小增长：
...
**比例 vs 绝对值悖论**：D3（55.5%）反而低于 C1/C3（62%），因 DSL 基线更高（7.47ms vs 1.5ms），固定开销占比被稀释。这印证了"DSL 越慢，劣化比例越低"的趋势——10M 下 DSL 基线升至 50-200ms 时，同等开销下占比将降至 1-5%。
...
#### 4. 10M 场景与大数据量趋势验证（5.4 节数据）

**10M 场景性能**：大结果集和小聚合开销比例与 1M 相近，但高基数聚合出现极端劣化：
...
> *C1-H → G3 不可直接对比：C1-H 用随机阈值 1-9000（扫描 ~50% 文档），G3 用时间过滤（扫描 ~16% 文档）。

- **大结果集**（A1-H→H3）：DSL +43%，SQL +13%，占比 ↓9.7pp ✅——序列化开销 ~80ms 固定，DSL 随数据量增长
- **大结果集+filter**（D3-H→G2）：DSL +88%，SQL +88%，占比持平 ✅
- **高基数聚合**（C1-H→G3）：不可直接对比，但 composite 开销与桶数相关（非文档数），C1-H/G3 桶数相近（~10K），composite 开销本应相近——占比差异来自 DSL 基线变化 ⚠️
- **小聚合**（C2-H→H2）：均缓存命中，占比持平 ≈

**假设验证**："大数据量劣化比例小"在**序列化主导场景**成立 ✅；在**聚合机制差异场景**无法验证 ⚠️（控制变量不同），但理论上 composite 开销 ∝ 桶数、DSL terms ∝ 文档数，大数据量下 DSL 增长更快占比应下降——需补充控制变量实验。
...
| 大数据量劣化比例小（聚合场景）  | ⚠️ 无法验证 | C1-H 83% → G3 88%（过滤条件不同） | composite ∝ 桶数，DSL terms ∝ 文档数，理论应下降 |
