...
| E1b-H | 两路UNION 250K行 | 139.17 | 143.47 | 148.76 | 155.97 | 139.29 | Calcite |
...
4. **查询简单**：A1/G0-1 查询仅含 1 个 WHERE 条件 + LIMIT，ANTLR 解析树节点数少；G0-2 聚合也仅 4 桶，Analyzer 工作量小
...
- **比例 vs 绝对值悖论**：D3 的比例（55.5%）反而**低于** C1/C3（62%），因为 D3 的 DSL 基线较高（7.47ms vs 1.5ms），固定开销占比被稀释。这印证了"DSL 越慢，SQL 劣化比例越低"的趋势
- **T 组趋势外推**：若数据量增至 10M，DSL 基线将升至 50-200ms（见 4.7 节），同等翻译开销下劣化比例将降至 1-5%——本测试的 D3 vs C1 对比已初步呈现此趋势

#### 4. G0 组纯翻译开销与端到端开销的差异
...
- **网络往返**：本测试 3 节点同机回环，网络延迟 <0.1ms。生产环境跨节点网络 0.5-2ms，差异会更大
- **G0 与端到端的交叉验证**：G0-1 ANALYZE 0.76ms + 序列化/调度 ~1.46ms ≈ A1 端到端开销 2.22ms，**数学闭合**，验证了"端到端开销 = 纯翻译 + 序列化 + 网络 + 排队"的分解模型
...
| E1b-H 两路UNION 250K行 | Calcite | 139.2 | 10000(LIMIT) | ⚠️ 较慢（流式拉取 250K 行后 LIMIT） |
...
- **E1b-H 最慢**（139.2ms）：两个子查询各返回 ~125K 行（status_code=500 占 5%，503 占 5%，共 100K 行），Calcite 流式合并后 LIMIT 10000。性能瓶颈是 **125K 行的拉取和合并**，而非 UNION 本身
...
**H1 极端异常深度分析**：
- **SQL 3913ms vs DSL 28ms（140 倍）**：SQL 路径将 `GROUP BY session_id ORDER BY cnt DESC LIMIT 5000` 转为 composite 聚合，需要流式拉取全部 ~100K 个 `session_id` 桶（每个桶含 count + avg_rt），在协调节点排序后取 Top 5000
- **DSL 28ms**：`terms` 聚合在每个分片层面排序取 Top 5000，协调节点仅合并 6 × 5000 = 30000 个桶，全程在 shard 层完成
- **根因**：SQL 插件的 `AggPushDownAction` 在 `ORDER BY cnt DESC LIMIT 5000` 时无法将 LIMIT 下推到 terms 聚合（composite 不支持 size 参数），导致全量桶拉取
- **影响**：任何 `GROUP BY <高基数字段>` + `ORDER BY <聚合字段>` + `LIMIT` 的 SQL 查询在高基数（>10K 桶）场景下都会出现此问题
- **缓解方案**：使用 DSL terms 聚合；或在 SQL 中添加 WHERE 条件降低基数；或使用 PPL `stats` 命令（走 Calcite pushdown）
...
- **高基数聚合反例**：C1-H → G3，DSL 从 13ms 降至 9ms（因 G3 有时间过滤），SQL 持平 78→74ms，开销占比反升。**但若控制变量（相同过滤条件），DSL 在 10M 上必然更慢，占比应下降**——本测试的 C1-H 用随机阈值、G3 用时间过滤，不可直接对比
- **结论修正**："大数据量劣化比例小"假设在**序列化主导**场景成立（大结果集），在**聚合机制差异**场景不成立（高基数聚合的 composite vs terms 差异不随数据量缩小）
